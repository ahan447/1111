...
@WebServlet("/board/*")
//서블릿은 WAS가 객체를 1개만 생성해서 사용하기 때문에 싱글톤 패턴을 적용하지 않아도 됨
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "insert":
			if ("GET".equals(request.getMethod())) {
				...
				// ..을 앞에 붙인 이유는 여기서 요청이 board/insert
		...
		case "list":
			// 서비스의 메소드 호
			List<Board2> list = service.boardList(request);
			// 포워딩 할 때 데이터를 request에 저장
			request.setAttribute("list", list);
			// 데이터를 읽어와서 출력할 때는 포워딩을 하는 것이 일반적
			RequestDispatcher dispatcher = request.getRequestDispatcher("../views/list.jsp");
			dispatcher.forward(request, response);
			break;
		case "detail":
			Board2 board = service.boardDetail(request);
			// 포워딩 할 때 데이터를 request에 저장
